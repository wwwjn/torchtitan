...
ENABLE_VLLM_TIMING = os.environ.get("VLLM_TIMING", "0") == "1"

# Global timing stats accumulator for summary
_timing_stats: dict[str, list[float]] = {}


def _log_vllm_timing(name: str, elapsed_ms: float, phase: str = ""):
    """Log timing information for vLLM operations.
    ...
    """
    if ENABLE_VLLM_TIMING:
        full_name = f"{phase}.{name}" if phase else name
        logger.info(f"[VLLM_TIMING] {full_name}: {elapsed_ms:.3f} ms")
        # Accumulate stats
        if full_name not in _timing_stats:
            _timing_stats[full_name] = []
        _timing_stats[full_name].append(elapsed_ms)
...
def print_timing_summary():
    ...
    if not stats:
        logger.info("[VLLM_TIMING] No timing data collected")
        return

    logger.info("\n" + "=" * 80)
    logger.info("[VLLM_TIMING] TIMING SUMMARY")
    logger.info("=" * 80)
    logger.info(
...
class GPUModelRunner(
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        Args:
            ...
            phase: Timing phase identifier (prefill/decode)
        ...
        if ENABLE_VLLM_TIMING:
            torch.cuda.synchronize()
            _log_vllm_timing(
                "_model_forward", (time.perf_counter() - forward_start) * 1000, phase
            )
        return result
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("gpu_model_runner: preprocess"):
            with self.synchronize_input_prep():
                ...
                # Determine phase: prefill if max tokens > 1, decode otherwise
                phase = "prefill" if max_num_scheduled_tokens > 1 else "decode"
                if ENABLE_VLLM_TIMING:
                    logger.info(
                        f"[VLLM_TIMING] Phase: {phase}, num_reqs: {num_reqs}, "
                        f"num_tokens: {num_tokens_unpadded}, max_scheduled: {max_num_scheduled_tokens}"
                    )
                ...
                if ENABLE_VLLM_TIMING:
                    ...
                    _log_vllm_timing(
                        "_prepare_inputs",
                        (time.perf_counter() - prepare_inputs_start) * 1000,
                        phase,
                    )
                ...
                build_attn_start = time.perf_counter()
                (
                    attn_metadata,
                    spec_decode_common_attn_metadata,
                ...
                if ENABLE_VLLM_TIMING:
                    ...
                    _log_vllm_timing(
                        ...
                        (time.perf_counter() - build_attn_start) * 1000,
                        phase,
                    )
        ...
        if ENABLE_VLLM_TIMING:
            torch.cuda.synchronize()
            _log_vllm_timing(
                "postprocess", (time.perf_counter() - postprocess_start) * 1000, phase
            )
        ...
        if ENABLE_VLLM_TIMING:
            ...
            _log_vllm_timing(
                "execute_model.total",
                (time.perf_counter() - execute_model_start) * 1000,
                phase,
            )
    ...
    def sample_tokens(
        ...
        sample_tokens_start = time.perf_counter()
        ...
        if ENABLE_VLLM_TIMING:
            torch.cuda.synchronize()
            _log_vllm_timing("_sample", (time.perf_counter() - sampling_start) * 1000)
    ...
    def _dummy_run(
        ...
        with self.maybe_dummy_run_with_lora(
            ...
            if self.use_aux_hidden_state_outputs:
                ...
                hidden_states = outputs  # shape(8192, 1024)
        ...
        )  # shape # shape(8192, 1024), (1024, 1024)
