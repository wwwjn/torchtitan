...
def _permute(w, n_heads_arg, dim1=None, dim2=None):
    ...
    if hasattr(tensor, "placements"):
        ...
        print(
            f"w1 tensor shape: {w1.shape}, placements: {w1.placements}, device_mesh: {w1.device_mesh}"
        )
        w1_full = w1.full_tensor()
        print(
            f"Are w0 and w1 tensors equal? {torch.allclose(w0_full.view(n_heads_arg, dim1 // n_heads_arg // 2, 2, dim2), w1_full, rtol=1e-5, atol=1e-5)}"
        )
        w2 = w1.transpose(1, 2)
        w2_full = w2.full_tensor()
        print(
            f"Are w1 and w2 tensors equal? {torch.allclose(w1_full.transpose(1, 2), w2_full, rtol=1e-5, atol=1e-5)}"
        )
        w3 = w2.reshape(dim1, dim2)
        w3_full = w3.full_tensor()
        print(
            f"Are w2 and w3 tensors equal? {torch.allclose(w2_full.reshape(dim1, dim2), w3_full, rtol=1e-5, atol=1e-5)}"
        )
        w4 = w3.clone()
        w4_full = w4.full_tensor()
        print(
            f"Are w2 and w3 tensors equal? {torch.allclose(w3_full.clone(), w4_full, rtol=1e-5, atol=1e-5)}"
        )
        return w4
    else:
...
def _view_test(w, n_heads_arg, dim1=None, dim2=None):
    ...
    if dim2 is None:
        dim2 = w.shape[1]
    return w.view(n_heads_arg, dim1 // n_heads_arg // 2, 2, dim2)
...
def test_permute_functions():
    ...
    print(
        f"Original tensor shape: {dist_tensor.shape}, placements: {dist_tensor.placements}, device_mesh: {dist_tensor.device_mesh}"
    )
    # Test _permute function
    permuted_tensor = _permute(dist_tensor, n_heads)
    print(f"After _permute:")
    print(
        f"Permuted tensor shape: {permuted_tensor.shape}, placements: {permuted_tensor.placements}, device_mesh: {permuted_tensor.device_mesh}"
    )
    ...
    are_tensors_equal = torch.allclose(
        original_full_permuted, permuted_full, rtol=1e-5, atol=1e-5
    )
...
def test_view_functions():
    ...
    # tensor_shape = [dim // 4, dim]
    tensor_shape = [dim // 4, dim]
    local_tensor = torch.randn(tensor_shape, device="cuda")
    print(f"Original tensor: {local_tensor}")
    # Create distributed tensor with specified sharding: _StridedShard(dim=0, sf=2), Shard(dim=0)
    placements = [_StridedShard(dim=0, split_factor=2), Shard(dim=0)]
    # NOTE: distribute_tensor does NOT support _StridedShard
    """
    dist_tensor = torch.distributed._tensor.distribute_tensor(
        local_tensor, device_mesh, placements
    )
    ...
    dist_tensor = DTensor.from_local(local_tensor, device_mesh, placements)
    full_tensor = dist_tensor.full_tensor()
    print(
        f"Original tensor shape: {dist_tensor.shape}, placements: {dist_tensor.placements}, device_mesh: {dist_tensor.device_mesh}"
    )
    # Test _view_test function
    permuted_tensor = _view_test(dist_tensor, n_heads)
    print(f"permuted_tensor: {permuted_tensor}")
    print(f"After _view_test:")
    print(
        f"After _view_test tensor shape: {permuted_tensor.shape}, placements: {permuted_tensor.placements}, device_mesh: {permuted_tensor.device_mesh}"
    )
    # Verify that _reverse_permute is the inverse of _permute
    original_full_permuted = _view_test(full_tensor, n_heads)
    permuted_full = permuted_tensor.full_tensor()
